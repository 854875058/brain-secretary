...
- 新增项目真源：`ops/project_registry.json`。
- 当用户提到“某个项目 / 某个仓库 / 铁塔多模态检索 / Tower-Eye”这类简称时，优先先查项目注册表，不要反复让用户补项目路径。
- 用户口中的“铁塔多模态检索项目”默认指：`https://github.com/854875058/Tower-Eye`。
- `Tower-Eye` 默认基线分支：`main`；当前首选工作分支：`feat/tower-eye-review-fix`。
- 对可恢复的阻塞（缺依赖、缺 `pytest`、缺 `numpy`、缺 `rg`、仓库未克隆、默认分支未确认、只缺本地临时环境）默认由内部 agent 自己补齐，不应把这类问题先甩回给用户。
- 正常闭环应该是：大脑派技术号实施 -> 验收号复核 -> 若发现问题或测试失败，大脑自动打回技术号继续修 -> 直到真的只剩授权/口令/业务偏好这类用户专属信息时，才询问用户。
